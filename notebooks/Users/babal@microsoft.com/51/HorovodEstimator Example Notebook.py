...
# MAGIC 
# MAGIC This notebook performs distributed fitting of a fully-connected deep neural network on MNIST data in a Spark DataFrame. The model training example is adapted from Uber's [tensorflow_mnist_estimator example script](https://github.com/uber/horovod/blob/9bdd70dad6d35fe7fa7c4b0875a27c5cf668ca83/examples/tensorflow_mnist_estimator.py).
# MAGIC 
...
# MAGIC 
# MAGIC To run the notebook, use Databricks Runtime ML.
...
# MAGIC %md Load a DataFrame containing MNIST data. In practice, the loaded DataFrame might be the output of some ETL done in Spark.
...
# MAGIC 
# MAGIC The tf.estimator-style `model_fn` ([see TensorFlow docs](https://www.tensorflow.org/api_docs/python/tf/estimator/Estimator)) works by:
...
# MAGIC %md Create a `HorovodEstimator` for distributed training of your model on a DataFrame.
...
# MAGIC %md Fit the estimator on the MNIST dataframe, then use the fitted model (a `TFTransformer` from Deep Learning Pipelines) to transform your data.
...
# MAGIC %md As stated above, you can restore our model from a checkpoint file to resume training. Here increase the `maxSteps` parameter of your estimator & run training again, starting from your previously-saved checkpoint:
...
# MAGIC Persist your model checkpoints & event files by copying them to DBFS, so that they can be accessed e.g. after the current cluster terminates.
...
dbutils.fs.ls("file:/tmp/horovod_estimator/")

# COMMAND ----------

dbutils.fs.rm("file:/tmp/horovod_saved/", recurse=True)
dbutils.fs.rm("/tmp/horovod_saved/", recurse=True)
...
sess = tf.Session()
export_path = "/tmp/horovod_saved/"

training_data = df['image']
...
with tf.Graph().as_default() as g:
    ...
    x = tfs.block(df, "image")

#x = tf.identity(df['image'], name='x')  # use tf.identity() to assign name
x = x
...
dbutils.fs.cp("file:/tmp/horovod_saved/", "dbfs:/tmp/horovod_saved/", recurse=True)

# COMMAND ----------

dbutils.fs.ls("dbfs:/tmp/horovod_saved/")

# COMMAND ----------

dbutils.fs.rm("/tmp/horovod_saved/", recurse=True)
dbutils.fs.rm("/tmp/horovod_saved1/", recurse=True)

# COMMAND ----------

dbutils.fs.cp("file:/tmp/horovod_saved/", "dbfs:/horovod_saved/", recurse=True)
...
display(dbutils.fs.ls("dbfs:/horovod_saved/"))

# COMMAND ----------

dbutils.fs.mount(
  source = "wasbs://deeplearning@dewsa.blob.core.windows.net/",
  mount_point = "/mnt/dl",
...
dbutils.fs.cp("dbfs:/tmp/horovod_saved/", "dbfs:/mnt/dl/horovod_saved/", recurse=True)

# COMMAND ----------

dbutils.fs.ls("dbfs:/mnt/dl/horovod_saved/")