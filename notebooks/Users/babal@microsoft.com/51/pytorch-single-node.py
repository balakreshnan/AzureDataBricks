...
# MAGIC %md ## Use PyTorch on a single node
# MAGIC 
...
# MAGIC 
# MAGIC This tutorial walks through a classic computer vision application of identifying handwritten digits. We train a simple Convolutional Neural Network on the MNIST dataset.

# COMMAND ----------

# MAGIC %md ### Parameters
# MAGIC While this notebook runs on both non-GPU and GPU clusters, we recommend using a GPU cluster. If you change the `cuda` parameter in the next code block to `cuda=True`, PyTorch will take advantage of CUDA-enabled GPU and run faster. 
# MAGIC 
...
# MAGIC 
# MAGIC The training algorithm goes through the training set for `epochs` passes. 
...
# MAGIC %md We can see key information about training in the output:
# MAGIC - At prescribed logging intervals, the training process prints out how many samples in the training set it has gone through in the current epoch, and the loss of the current training mini-batch. 
# MAGIC - We can see that over the epochs, on the testing set, the loss decreases and the accuracy increases, suggesting that the model is indeed improving, even though at batch level there might be fluctuations. 