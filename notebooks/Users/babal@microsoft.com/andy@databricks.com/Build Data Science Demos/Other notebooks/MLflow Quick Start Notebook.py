...
# MAGIC 
# MAGIC To run a long-lived, shared MLflow tracking server, you can launch an Azure instance to run the [MLflow Tracking server](https://mlflow.org/docs/latest/tracking.html). To do this:
# MAGIC 
# MAGIC * Create an **Anaconda with Python 3** Azure instance 
# MAGIC   * Install MLflow
# MAGIC   * Open port 5000 for MLflow server
# MAGIC 
...
# MAGIC   * Run the `server` command in MLflow passing it your `artifact-root` and `--host 0.0.0.0`, e.g. `mlflow server --artifact-root azure  --host 0.0.0.0`.
