...
display(vectorizer.transform(dataset))
...
var variables = Array
coefficents.sortByKey().collect().foreach({
  case (weight, name) =>
  { 
        val symbol = if (weight > 0) "+" else "-"
        val absWeight = Math.abs(weight)
        equation += (s" $symbol (${absWeight} X ${name})")
  }
}
)
...
display(lrModel.stages.last.asInstanceOf[LinearRegressionModel], predictionsAndLabels.withColumnRenamed("Predicted_PE", "Predicted_PE_Old"), "fittedVsResiduals")
...
// MAGIC %md The line above will pull the Decision Tree model from the Pipeline as display it as an if-then-else string
